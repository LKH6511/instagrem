...
//temporary data
const my_data = [
  { user_id: "lkh6511", profile_pic: "img/profile/lkh.jpg" },
  { user_id: "bcat", profile_pic: "img/profile/bcat.jpeg" },
  { user_id: "duck", profile_pic: "img/profile/duck.jpeg" },
  { user_id: "gcat", profile_pic: "img/profile/gcat.jpeg" },
  { user_id: "ggcat", profile_pic: "img/profile/ggcat.jpeg" },
  { user_id: "leekh", profile_pic: "img/profile/ccat.jpeg" },
...
function search_id() {
  ...
  const filtered_data = my_data.filter((user) =>
    user.user_id.includes(input_value)
  );
  //필터된 데이터를 추가함
...
function controll_navbar_search() {
  ...
  const navbar_search_container = document.querySelector(
  ...
  navbar_input.addEventListener("keyup", () => {
    if (navbar_input.value.trim().length)
      navbar_search_container.classList.add("navbar-visible");
    search_id();
  });
}

//프로필 메뉴바 밖 클릭했을때 꺼잠 아님켜지고..
function controll_navbar_profile() {
  document.addEventListener("click", (evt) => {
    const navbar_profile_container = document.querySelector(
    ...
    if (is_clicked_container || is_clicked_profile_pic)
      navbar_profile_container.classList.add("navbar-visible");
