const tmp_data = [
  {
    user_id: "lkh6511",
    user_profile: "img/profile/lkh.jpg",
    story: true,
    img_src: "img/feedimg/leekh.jpg",
    content: `ㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇㅇㅅㅇㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇㅅㅇ`,
    likes: 21,
    comments: [
      { user_id: "djdjdj", content: "와~와~와~와~와~와~와~" },
      {
        user_id: "ddvsssd",
  ...
  {
    user_id: "leekh",
    user_profile: "img/profile/ccat.jpeg",
    story: true,
    img_src: "img/profile/ccat.jpeg",
    content: "뫄~",
    likes: 21,
    comments: [
      { user_id: "loxax", content: "zzzz" },
      { user_id: "ccat", content: "ㅡㅡ!!" },
    ],
...
function get_date_difference(postday) {
  ...
  post_upload_date = new Date(
    post_upload_date[0],
    post_upload_date[1] - 1,
    post_upload_date[2]
  );
...
function controll_show_more_btn(evt, slice_length) {
  ...
  if (evt.target.innerHTML === "더 보기") {
    ...
    original_content.innerHTML = original_content.innerHTML.slice(
      0,
      slice_length
    );
...
function hide_feed_content() {
  const more_btn = document.querySelectorAll(
    ".feed-content .feed-content-morebtn"
  );
  const feed_contents = document.querySelectorAll(".feed-content .content");
  const feed_hidecontents = document.querySelectorAll(
    ".feed-content .content-hide"
  );
  const slice_length = 46;
  feed_contents.forEach((content, idx) => {
    if (content.innerHTML.length > slice_length) {
      feed_hidecontents[idx].innerHTML = `${content.innerHTML.slice(
        slice_length
      )}`;
